...
from net.densenet import densenet169
try:
...
class CoseIns(torch.nn.Module):
    def __init__(self, s=128.0, m=0.20):
        super(CoseIns, self).__init__()
        self.s = s
        self.m = m
...
class AdaINGen(nn.Module):
    # Improved AdaIN auto-encoder architecture
    def __init__(self, input_dim, params, fp16):
        super(AdaINGen, self).__init__()
        dim = params['dim']
        n_downsample = params['n_downsample']
        ...
        # Improved decoder with self-attention and multi-scale features
        self.dec = ImprovedDecoder(n_downsample, n_res, self.output_dim, 3, dropout=dropout, res_norm='adain', activ=activ, pad_type=pad_type, non_local=True, fp16=fp16, use_cgan=True)

        # MLP to generate AdaIN parameters
        self.mlp = MLP(id_dim, 8 * self.output_dim, mlp_dim, 3, norm=mlp_norm, activ=activ)
    ...
    def assign_adain_params(self, adain_params, model):
        ...
        for m in model.modules():
            if m.__class__.__name__ == "AdaptiveInstanceNorm2d":
                mean = adain_params[:, :dim].contiguous()
                std = adain_params[:, dim:].contiguous()
                m.bias = mean.view(-1)
                m.weight = std.view(-1)
                adain_params = adain_params[:, 2 * dim:]
...
class ImprovedDecoder(nn.Module):
    def __init__(self, n_upsample, n_res, dim, output_dim, dropout=0, res_norm='adain', activ='relu', pad_type='zero', non_local=True, fp16=False, use_cgan=True):
        ...
        self.final_layers = nn.Sequential(
            ...
            Conv2dBlock(dim, dim, 3, 1, 1, norm='none', activation=activ, pad_type=pad_type),
            Conv2dBlock(dim, output_dim, 1, 1, 0, norm='none', activation='none', pad_type=pad_type)
        )

    def forward(self, x):
        x_initial = self.model(x)

        for upsample_layer in self.upsample_layers:
            x_initial = upsample_layer(x_initial)

        x_att = self.self_attention(x_initial)

        # Combine the initial features and the self-attention features
        x_combined = x_initial + x_att

        output = self.final_layers(x_combined)
        return output
